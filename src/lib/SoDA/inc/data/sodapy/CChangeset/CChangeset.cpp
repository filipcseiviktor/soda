...
CChangeset::CChangeset() :
    m_codeElements(new CIDManager()),  //CIDManager.h
    m_changes(new CRevision<IBitList*>()),  //CRevision.h,  IBitList.h
...
{
    if(m_createChanges) {
        IntVector revNums = m_changes->getRevisionNumbers();  //getRevisionNumbers() -->CRevision.h
        for(IndexType i = 0; i < revNums.size(); i++) {// CIDManager.h, CIndexBitlist.h, CRevision.h, IBitList.h, IIDManager.h
            delete m_changes->getRevision(revNums[i]);  //Crevision.h
...
IntVector CChangeset::getRevisions(const String& codeElementName) const
{
    ...
    for(IndexType i = 0; i < revNums.size(); i++) {
        if(m_changes->getRevision(revNums[i])->at((*m_codeElements)[codeElementName]))  //at()-->IBitList.h
            result.push_back(revNums[i]); //CIndexBitlist., IBitList.h
    }
    return result;
}
...
{
    for(IntVector::const_iterator it = revisionNumbers.begin(); it != revisionNumbers.end(); ++it) { //begin(),end()-->CIndexBitList.h, IBitList.h
        addRevision(*it);
    }
}
...
{
    if (!m_changes->revisionExists(revNum))  //CRevision.h
        return;
