...
public:
    ...
    {
        if(col != p->size()-1) {  //IBitList.h (size())
            col++;
        } else {
            ++p;    // next row
...
void CBitMatrix::rowCounts(std::vector<IndexType> &v) const
{
    ...
    for (IndexType r = 0; r < m_row; r++) {
        v[r] = m_data[r].count();           //count()-->CBitList.h,  IBitList.h
    }
}
...
void CBitMatrix::toggleValue(IndexType row, IndexType col)
{
    if(row >= m_row || row < 0)
      //  throw CException("soda::CBitMatrix", "Index out of bound!");
    if(m_data[row].at(col)) {                                       //at() --> CBitList.h IBitList.h
...
{
    CBitList* tmp;
    tmp = new CBitList[newRow]();               //CBitList.h
    ...
    for(IndexType i = 0; i < rowCounter; i++){
        for(IndexType j = 0; j < colCounter; j++) {  //push_back() CBitList.h, IBitList.h
