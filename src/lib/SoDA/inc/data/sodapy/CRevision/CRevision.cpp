...
{
    return m_data->count(rev) > 0;  //count()-->IBitList.h
}

template <typename T>
T& CRevision<T>::getRevision(const RevNumType rev)
{
    return m_data->at(rev);  //at()-->IBitList.h
}
...
{
    m_data->erase(revNum); //erase()-->IBitList.h
}
...
IntVector CRevision<T>::getRevisionNumbers() const
{
    ...
    for (typename std::map<RevNumType, T>::iterator it = m_data->begin(); it != m_data->end(); ++it) {   //begin(), end(), IBitList.h
        tmp.push_back(it->first);  //push_back() -->IBitList.h
    }
...
template <typename T>
typename std::map<RevNumType, T>::size_type CRevision<T>::size() const //IBitList.h size()
{
    return m_data->size();
}
...
{
    out->writeInt4(chunk);//CBinaryIO (writeInt4)
    out->writeULongLong8(size() * (4 + sizeof(T)));   //CBinaryIO (writeULongLong8)

    out->writeUInt4(size());  //CBinaryIO
    for (typename std::map<RevNumType, T>::iterator it = m_data->begin(); it != m_data->end(); ++it) {
        out->writeUInt4(it->first);
        out->writeData(&(it->second), sizeof(T)); //CBinaryIO
    }
}
...
{
    m_data->clear();  //IBitList

    for (RevNumType size = in->readUInt4(); size > 0; size--) {   //CBinaryIO
        RevNumType key = in->readUInt4();
        T value;
        in->readData(&value, sizeof(T)); //CBinaryIO
        (*m_data)[key] = value;
    }
}
