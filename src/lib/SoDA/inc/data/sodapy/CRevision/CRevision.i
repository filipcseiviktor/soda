...
%extend soda::CRevision<T> {
    T& __getitem__(const int rev) {
        return (*$self)[rev];
     }
};
...
/*CSoDAio */ 
	...
	/* a cel, hogy a python oldalon a modul importalasa utan pl. ne "crevision.RELATION" -kent kelljen hivatkozzunk, hanem lehessen a kovetkezokepp:
